...
public class STGCMulticastSockets extends MulticastSocket{
	...
	public void send(DatagramPacket packet) {
	    ...
	    DatagramPacket cipherPacket = new DatagramPacket(cipherdata, cipherdata.length, packet.getAddress(), 
	    		packet.getPort());
	    try {
			super.send(cipherPacket);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private byte[] ReadMessage(byte[] messagedata) throws Exception {
		DataInputStream istream = 
		          new DataInputStream(new ByteArrayInputStream(messagedata));
